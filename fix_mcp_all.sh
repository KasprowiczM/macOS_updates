...
# ============================================================
# Finds all MCP config files and applies absolute paths and PATH env.
...
# Find MCP config files in common locations (~/.gemini, ~/.codeium, projects, etc.)
...
while IFS= read -r -d '' f; do
    CONFIG_FILES+=("$f")
done < <(find "$HOME" -maxdepth 4 \( -name "mcp_config.json" -o -name ".mcp.json" \) -print0 2>/dev/null)
