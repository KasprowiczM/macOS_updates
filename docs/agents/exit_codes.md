...
| Script | Code | Meaning |
|--------|------|---------|
| `update_all.sh` | `0` | All steps completed without critical failure |
...
With `MAC_UPDATE_DRY_RUN=1` or `bash update_all.sh --dry-run`, leaf scripts print `[DRY-RUN]` and exit **0** without mutating the system.
