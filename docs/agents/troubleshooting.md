...
| Status: quit the app so its updater can install | The app update was staged/downloaded in the background by its vendor updater, but cannot finish installing while running. The app was already open when the run started, so the toolkit intentionally left it untouched ("Apps you have open are never closed"). Quit the app and re-run `update_all.sh` to allow installation |
| Why did the toolkit open an app and close it again? | The toolkit only launches apps that were not already running, in order to trigger their built-in vendor updaters. It only quits what it launched itself (`toolkit_launched.txt`); apps you already have open are never closed |
| Chrome: staged rollout | Google Chrome is in a staged cohort rollout: public version is newer on Google VersionHistory, but Omaha updater has not offered it to this Mac yet (`rollout_hold`). Informational, no soft failure — update will apply once offered |
| Google status ✅ with a time | GoogleUpdater recorded a recent `noupdate` check within `MAC_UPDATE_OMAHA_MAX_AGE_H` hours (default 6h); the toolkit accepts this timestamped proof as current without re-waking the updater daemon |
| App Store update stays pending after retry | Track 2 GUI update was attempted and retried, but the App Store did not finish installing. Update the app manually in the Mac App Store |
| Cursor installs from the vendor without launching | Configured in `config/vendor_direct_first.txt`: Cursor skips the silent launch and stage-wait cycle when behind, downloading and installing directly from the verified vendor feed |
