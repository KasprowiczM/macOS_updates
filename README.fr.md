...
> **v1.2.0** — Orchestrateur de mise à jour prêt pour la production pour **Mac Apple Silicon sous macOS 13+**. Coordonne les mises à jour vérifiées pour les logiciels installés. **Multilingue** (7 langues). déclenchements intégrés — **uniquement pour les logiciels déjà installés**. **Multilingue** (7 langues). Couche privée optionnelle via [`dev_sync/`](dev_sync/README.md).
